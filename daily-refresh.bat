...
if exist "C:\Users\wisdom\Documents\Codex\AI-Launcher\Start-Gateway-Stack.ps1" (
  ...
  echo %LOG_PREFIX% 网关就绪检查完成 (等待约 !WAIT_SEC! 秒)
)
...
REM ===== 4. 爬取 + AI 翻译 + 生成报告（约1-3分钟）=====
.venv\Scripts\python.exe -m trendradar
if errorlevel 1 goto :failed

REM ===== 5. 富化（单次最多30篇，异步后台执行）=====
set "RSSYXY_AI_LIMIT=30"
start "RSSYXY-Enrich" /b cmd /c ".venv\Scripts\python.exe tools\enrich_articles.py >> output\enrich.log 2>&1"
echo %LOG_PREFIX% 后台富化已启动（单次30篇，不阻塞发布）
